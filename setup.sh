...
function show_menu() {
    ...
    clear
    figlet "ZIVPN KINGSTORE" | lolcat
    
    echo -e "${YELLOW}╔═══════════// ${CYAN}PT KINGSTORE PREMIUM${YELLOW} //═════════════╗${NC}"
    _draw_info_panel
...
echo "✅ Instalasi selesai. Service ZiVPN harusnya aktif dan panel bisa mendeteksi."
    ...
    wget -O /usr/local/bin/zivpn_helper.sh https://raw.githubusercontent.com/kingstorevpn/zivpn/main/zivpn_helper.sh
    if [ $? -ne 0 ]; then
